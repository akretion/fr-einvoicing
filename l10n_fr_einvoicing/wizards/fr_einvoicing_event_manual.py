...
class FrEinvoicingEventManual(models.TransientModel):
    ...
    status_sale = fields.Selection("_status_sale_selection", string="Status for Customer Invoice/Refunds")
    status_purchase = fields.Selection("_status_purchase_selection", string="Status for Vendor Bills/Refunds")
    ...
    attachment_ids = fields.Many2many(
        'ir.attachment', 'fr_einvoicing_event_manual_attachment_rel', string="Attachments")
    ...
    def _prepare_event_vals(self):
        self.ensure_one()
        status = self.move_id.is_sale_document() and self.status_sale or self.status_purchase
        vals = {
            'status': status,
            'direction': 'out',
    ...
    def run(self):
        ...
        if self.detail_required:
            if not self.detail_ids:
                raise UserError(self.env._('For this status, you must create at least one line of details.'))
        if self.confirm_required and not self.confirm:
            if status == "refused":
                raise UserError(self.env._("You must confirm the refusal of the vendor bill/refund, or click on cancel. Reminder: the refusal is definitive. On the contrary, if you want to block the vendor bill temporarily, you should create an event 'Disputed'."))
            else:
                raise UserError(self.env._("You must confirm the creation of the event or click on cancel."))
        event = self.env['fr.einvoicing.event'].sudo().create(self._prepare_event_vals())
        self.move_id.message_post(body=Markup(self.env._("Event <a href=# data-oe-model=fr.einvoicing.event data-oe-id=%(event_id)s>%(event)s</a> created.", event=event.display_name, event_id=event.id)))
        if status == "refused":
